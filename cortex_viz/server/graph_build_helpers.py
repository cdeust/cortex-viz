...
def _set_progress(**kw) -> None:
    ...
    Stamps ``phase_started_at`` on every phase TRANSITION (via
    ``state.stamp_phase_change``, on this process's own monotonic clock)
    so ``get_build_progress`` can derive a per-phase ``phase_elapsed`` at
    ...
    with state._build_progress_lock:
        ...
        state.stamp_phase_change(state._build_progress, kw)

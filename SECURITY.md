...
cortex-viz is a **read-only** visualization bridge over Cortex's data, but it
does two things that shape its threat model, stated plainly here:
...
- **It reads the Cortex store and your artifacts.** It connects to the shared
  ...
  build the graphs. It does not write to that store (see `PRIVACY.md`).
...
- **UI fingerprint** — `cortex-viz-ui-manifest.sha256` is a `sha256sum` of
  every file under `ui/`, so the exact JS your browser executes is pinned to
  the tagged commit and diffable against the previous release.

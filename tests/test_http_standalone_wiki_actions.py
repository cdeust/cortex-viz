...
def test_page_with_sources_but_no_matching_activity(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "load_page_sources",
        lambda store, pid: [
            {"source_path": "foo.py", "link_kind": "documents", "confidence": 1.0}
        ],
    )
    monkeypatch.setattr(
        wiki_page_actions_mod, "resolve_file_node_id", _fake_resolve_file_node_id
    )
    monkeypatch.setattr(
        activity_store_mod, "find_by_target_ids", lambda store, ids, limit=2000: []
    )
    monkeypatch.setattr(
        activity_store_mod, "scan_legacy_file_rows", lambda store, limit=2000: []
    )
    got = _page_actions(store=None, params={"page_id": "7"})
    assert got["sources"] == [
        {"source_path": "foo.py", "link_kind": "documents", "resolved": True}
    ]
    assert got["actions"] == []
    assert got["count"] == 0
...
def test_limit_is_bounded_and_reports_truncation(monkeypatch):
    ...
    monkeypatch.setattr(
        activity_store_mod, "find_by_target_ids", lambda store, ids, limit=2000: rows
    )
    monkeypatch.setattr(
        activity_store_mod, "scan_legacy_file_rows", lambda store, limit=2000: []
    )
